...
const SearchCategoryScreen = () => {
    ...
    const renderCategory = ({item, index}) => (
        <Card color={(index + 1) % 2 === 0 ? greenWheel(Math.round((index + 1) / 2)) : blueWheel(Math.round((index + 1) / 2))} style={styles.categoryCard} rounded onPress={() => navigation.navigate('Specialties', {category: item})}>
